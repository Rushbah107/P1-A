...
function resetBoard() {
    for(var i=0; i<document.querySelectorAll(".col").length; i++) {
...
function playerMover(value, row, col) {
    ...
    for(var i=0; i<3; i++) {
        if(matrix[i][0] == matrix[i][1] && matrix[i][1] == matrix[i][2]) victory = matrix[i][0];
        if(matrix[0][i] == matrix[1][i] && matrix[1][i] == matrix[2][i]) victory = matrix[0][i];
    }
